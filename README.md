...
UI (Frontend)
     │  Request / Response
     ▼
Clinic App (FastAPI + LLM Orchestration)
     │
     ├── Maintains in-memory user chat history
     │
     ├── Sends user query + minimal context to the LLM
     │        └─ LLM may return:
     │              • A final response, or
     │              • A tool/function call request
     │
     └── If tool call required → executes internal service logic
               │
               ▼
       Clinic Services (Appointment & Availability)
               │
               ▼
        MySQL Database (super_clinic)
